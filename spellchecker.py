...
def spellchecker():
    ...
    txt = open('result.txt') 
    arrayTextRecognition = re.split(" |\n", txt.readline() )
    text =''
    for i in range(len(arrayTextRecognition)):
        short_circuit_result = suggestions(arrayTextRecognition[i], real_words, short_circuit=True)
        text = [ (x, word_model[x]) for x in short_circuit_result ][0][0]
        if(len(short_circuit_result) == 1):
            for x in short_circuit_result:
                writeFile("result-spell-checked.txt", x)
                writeFile("result-spell-checked.txt", " ")
        else:
            writeFile("result-spell-checked.txt", "(")
            for x in short_circuit_result:           
                writeFile("result-spell-checked.txt", x)
